...
def transaction_hashes_to_transactions(dodgy_transaction_hash_list, transactions_df, name):
    ...
    for transaction_hash in dodgy_transaction_hash_list:
        if transaction_hash != start_loop:
            ...
            transaction['loop_id'] = name.upper() + str(loop_id)
...
def filter_by_timestamp(datadf, name):
    loop_ids = datadf.loop_id.unique()
    ...
    for loop_id in loop_ids:
        timestamps = datadf.loc[datadf['loop_id'] == loop_id, 'timestamp']
        ...
        if((delta.days > 7)):
            datadf = datadf[datadf.loop_id != loop_id]
        if(delta_first_transaction.days < 30):
            num_loops_first_month += 1
    datadf.sort_values("loop_id", inplace = True) 
    datadf.to_csv("possible_wash_trades/" + name + ".csv")
...
def create_loops():
    ...
    for name, collection in collection_dict.items():
        ...
        # datadf is a df of suspect transactions for a single collection
        filter_by_timestamp(datadf, name)

if __name__ == '__main__':
    create_loops()

# loopid fromaddress toaddress tokenid ethprice date transhash